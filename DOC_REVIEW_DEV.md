...
**État des lieux (5ᵉ passe) :** Une relecture complète du code a été effectuée après application des correctifs. Tous les points des revues précédentes sont bien en place. La section 4 dresse le bilan. La section 4.1 ajoute la vérification de la 5ᵉ révision et d’éventuels points optionnels. La section 3 est conservée comme référence historique.
...
**Fichier :** `src/corpusstudio/app/ui_mainwindow.py`  
**Méthode :** `_setup_logging_for_project`

**Problème :** Le handler de fichier est ajouté au logger `corpusstudio`, mais la suppression est faite sur le logger **racine**. Les anciens `FileHandler` ne sont jamais retirés → à chaque ouverture de projet, un nouveau handler s’accumule (fuite).
...
**Fichier :** `src/corpusstudio/app/ui_mainwindow.py` (lignes 177–178)  
**Fichier concerné :** `src/corpusstudio/core/storage/project_store.py`
...
**Fichier :** `src/corpusstudio/core/storage/db.py`  
**Méthode :** `query_kwic`
...
**Fichier :** `src/corpusstudio/core/normalize/profiles.py`  
...
**Fichier :** `src/corpusstudio/app/workers.py`
...
| Élément | Statut |
|--------|--------|
| `_setup_logging_for_project` | ✅ Utilise `corpus_logger = logging.getLogger("corpusstudio")` pour add/remove. |
...
- **`query_kwic`** : Si la fonction est un jour appelée avec un terme vide (en dehors de l’UI), `_fts5_match_query("")` produit `'""'` et FTS5 peut réagir de façon imprévisible. L’UI filtre déjà avec `if not term: return`. Pour plus de robustesse, on peut ajouter en tête de `query_kwic` : `if not term or not term.strip(): return []`.
- **`subslikescript._make_soup`** : Le `except Exception:` (fallback lxml → html.parser) n’est pas loggé. Acceptable pour un fallback silencieux ; optionnel : logger en debug.
...
| Priorité | Réf   | Sujet |
|----------|-------|--------|
| Haute    | § 3.1 A | Gestion des handlers de log (éviter la fuite, cibler le logger `corpusstudio`). |
...
|---------|--------------------------|
| `src/corpusstudio/app/ui_mainwindow.py` | Handlers de log, `load_project_config`, signal error, constantes TAB_*, champs à l’ouverture, TextEditHandler, imports, type hint |
| `src/corpusstudio/core/storage/project_store.py` | `load_project_config`, `load_episode_transform_meta` / `get_episode_transform_meta_path` |
| `src/corpusstudio/core/storage/db.py` | FTS5 dans `query_kwic`, import datetime en tête |
| `src/corpusstudio/core/normalize/profiles.py` | Suppression de `span_start` |
| `src/corpusstudio/app/workers.py` | Avertissement si thread ne se termine pas dans le délai |

---

*Document généré à partir de la revue de code du projet HowIMetYourCorpus (5ᵉ révision).*
