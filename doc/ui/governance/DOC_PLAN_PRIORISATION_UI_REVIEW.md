...
- Resolution de l'etat UI global des actions scope (preconditions index/store/ids) centralisee dans le controller.
...
- Snapshot workflow (`counts + error_ids + advice`) centralise dans le controller et reutilise dans `refresh()` Corpus.
- Formatage de la ligne de statut workflow centralise dans le controller.
...
3. Jalon release UI fige et publie: `v0.5.2-ui-jalon-20260215-r1` (exe + app zip), details dans `NOTES_RELEASE_UI_JALON_2026-02-15.md`.
4. Stabiliser les runs `UI Smoke Tests` sur GitHub runner (investigation des echecs recents en CI).
